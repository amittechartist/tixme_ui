...
const Dashboard = ({ title }) => {
    ...
    const getStatusForEventId = (eventId) => {
        const foundObject = Requestlist.find(obj => obj.event_id === eventId);
        if (foundObject) {
            console.warn(foundObject.status);
            return foundObject.status;
        }

        // Return a default value or handle the case when no matching object is found
        return null;
    };
    const HandelPayoutRequest = (id) => {
        MySwal.fire({
            title: 'Are you sure you want to send payout request?',
            showDenyButton: true,
            ...
            if (result.isConfirmed) {
                HandelPayoutSubmit(id);
            } else if (result.isDenied) {
            }
        })
    }
    const HandelPayoutSubmit = async (id) => {
        try {
            const requestData = {
                id: id,
                organizerid: organizerid
            };
            fetch(apiurl + 'event/send-payout-request', {
                method: 'POST',
                headers: {
    ...
    const getMyrequest = async () => {
        try {
            ...
            fetch(apiurl + 'event/get-payout-request-list', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json', // Set the Content-Type header to JSON
                },
                                                    ...
                                                    <>
                                                        {Listitems.map((item, index) => (
                                                            <Col md={12} className="event_list_box_main">
                                                                {getStatusForEventId(item._id) >= 0 ? (
                                                                    <>
                                                                        {getStatusForEventId(item._id) == 0 ? (
                                                                            <button className="list-active-ticket-btn" type="button">Request Pending</button>
                                                                        ) : (
                                                                            <button className="list-active-ticket-btn" onClick={() => HandelPayoutRequest(item._id)} type="button">Request Payout</button>
                                                                        )
                                                                        }
                                                                    </>
                                                                ) : (
